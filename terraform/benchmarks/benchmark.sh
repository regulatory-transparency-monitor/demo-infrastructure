...
if ! command -v stress &> /dev/null; then
    echo "stress command not found! Installing..."
    sudo apt-get update
    sudo apt-get install -y stress
fi

# Stress CPU intermittently in the background
while true; do
    stress --cpu 2 --timeout 20s  # Stress for 20 seconds
    sleep 10s  # Rest for 10 seconds
done &
STRESS_PID=$!
...
# Stop the background CPU stress
kill $STRESS_PID
